...
class PlaneNearestPointsFitter:
    ...
    def process(self, k):
        ...
        :return: The k-nearest points, direction vector of the fitted line, a point on the line, and t_range.
        """
        k_nearest_points = self.find_k_nearest_points(k)
        direction_vector, point_on_line, t_range = self.fit_line_to_points(k_nearest_points)
        return k_nearest_points, direction_vector, point_on_line, t_range
    ...
    def fit_line_to_points(self, points):
        ...
        # Calculate an appropriate range for t based on the scale of the point cloud
        point_cloud_range = np.ptp(self.point_cloud, axis=0)  # Range of the point cloud in each dimension
        t_range = np.max(point_cloud_range) / np.linalg.norm(direction_vector)  # Scale t range by the data spread

        return direction_vector, point_on_line, t_range

    def visualize(self, k_nearest_points, direction_vector, point_on_line, t_range):
        """
        ...
        :param t_range: The range for the parameter t to scale the fitted line.
        """
